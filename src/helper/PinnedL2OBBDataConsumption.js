...
export default function PinnedL2OBBDataConsumption(DCParms){
  console.log("DCParms", DCParms);
  console.log("DCParms", DCParms.parms.canID);
  let period = DCParms.parms.period;
  let CanId = DCParms.parms.canID;
  ...
  useEffect(() => {
    console.log("networkParms.period",period);
  ...
  // });
   ...
   // const labels = data.map(item => item.startDt.substring(0, 10));
    ...
    const sortedData = data.sort((a, b) => new Date(b.startDt) - new Date(a.startDt));
    const totalDataByDate = {};
    sortedData.forEach(item => {
      const date = new Date(item.startDt);
      const dateString = `${date.getDate()} ${months[date.getMonth()]}`;
    
      // if (totalDataByDate[dateString]) {
      //   totalDataByDate[dateString] += convertBytesToMbps(parseFloat(item.total));
      //   console.log("totalDataByDate",totalDataByDate);
      // } else {
      //   totalDataByDate[dateString] = convertBytesToMbps(parseFloat(item.total).toFixed(2));
      // }
      const convertedTotal = parseFloat(convertBytesToMbps(item.total)); // Ensure a numerical value
  
  if (!isNaN(convertedTotal)) {
    if (totalDataByDate[dateString]) {
      totalDataByDate[dateString] += convertedTotal;
    } else {
      totalDataByDate[dateString] = convertedTotal;
    }
  }
    });
    Object.keys(totalDataByDate).forEach(dateString => {
      totalDataByDate[dateString] = totalDataByDate[dateString].toFixed(2);
    });
